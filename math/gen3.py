...
Key design choices:
- Defaults to a BASE math model rather than an instruction-tuned chat model.
- Defaults to MATH-style datasets rather than raw HMMT for 7B-class models.
- Uses a pilot -> filter -> final pipeline to explicitly target the steep pass@k regime.
- Handles both ordinary Hugging Face datasets and multi-config datasets such as
  EleutherAI/hendrycks_math.
- Makes answer extraction and level parsing more robust.

Example:
    python run_math_passk_calibrated.py \
...
def derive_gold_answer(row):
    ...
    if direct is not None:
        ...
        boxed = extract_last_boxed(direct)
        if boxed is not None:
            return boxed
        return direct

    solution = maybe_get(row, "solution")
    if solution is not None:
        boxed = extract_last_boxed(str(solution))
        if boxed is not None:
            return boxed
...
def load_math_problems(dataset_name: str, split: str, levels=None, subjects=None, limit=None, seed=42):
    ...
    subject_filter = set(str(x) for x in subjects) if subjects is not None else None
    ...
    for idx, row in enumerate(ds):
        ...
        subject_str = str(subject).strip() if subject is not None else None
...
def main():
    ...
    problems = load_math_problems(
        ...
        limit=args.candidate-limit if False else args.candidate_limit,
        seed=args.seed,
    )
    if not problems:
        sys.exit(
            "[fatal] no problems loaded after filtering. "
            "Try removing --levels first, or specify --subjects for hendrycks_math."
        )
    ...
    pilot_records, pilot_summary = run_stage(
        llm=llm,
        problems=problems,
        stage_name="pilot",
    ...
    if not selected:
        ...
        print("[select] or changing subjects/temperatures.")
        return
